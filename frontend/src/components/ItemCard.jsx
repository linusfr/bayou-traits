      ...
      {isWeapon ? (
        <>
          ...
          <div className="flex items-center gap-2 mt-2 flex-wrap">
            {item.weapon_class && item.weapon_class !== "unknown" && (
              <span
                className={`text-xs font-medium capitalize ${WEAPON_CLASS_COLOR[item.weapon_class] ?? "text-hunt-text-muted"}`}
              >
                {item.weapon_class}
              </span>
            )}
            {item.size > 0 && (
              <span className="text-xs text-hunt-text-dim">
                {item.size}-slot
              </span>
            )}
          </div>
        ...
        <>
          ...
          <div className="flex items-center gap-2 mt-2 flex-wrap">
            {item.tool_class && (
              <span
                className={`text-xs font-medium capitalize ${TOOL_CLASS_COLOR[item.tool_class] ?? "text-hunt-text-muted"}`}
              >
                {item.tool_class}
              </span>
            )}
        ...
        <>
          ...
          <div className="flex flex-wrap gap-1 mt-2">
            {item.category && item.category !== "unknown" && (
              <span
                className={`inline text-xs px-2 py-0.5 rounded border capitalize
            ...
            {item.trait_type &&
              ...
              TRAIT_TYPE_PILL[item.trait_type] && (
                <span
                  className={`inline text-xs px-2 py-0.5 rounded border capitalize ${TRAIT_TYPE_PILL[item.trait_type]}`}
                >
                  {item.trait_type}
                </span>
              )}
            {item.is_burn && (
              <span
                className={`inline text-xs px-2 py-0.5 rounded border capitalize ${BURN_PILL}`}
              >
                burn
              </span>
            )}
            {item.is_solo && (
              <span
                className={`inline text-xs px-2 py-0.5 rounded border capitalize ${SOLO_PILL}`}
              >
                solo
              </span>
            )}
